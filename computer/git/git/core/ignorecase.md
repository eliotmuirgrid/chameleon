...
3. **This knob** — `core.ignorecase` tells Git whether the **current** working tree filesystem is [case-insensitive](../letter_case/case_insensitive_comparison.md) for collision purposes, so Git can align **index vs disk** behavior and avoid silent corruption. It is **not** a style switch; it reflects **environment**. Vocabulary hub: [Letter case (for paths and names)](../letter_case.md).
...
**What leaks when `core.ignorecase` is true:** Git tracks the mismatch between how **Git** must think about path strings for **portability** and how some **filesystems** treat [letter case](../letter_case.md). On typical **Windows** and **macOS** disks, names are [case-insensitive](../letter_case/case_insensitive_comparison.md): `file.txt` and `File.txt` are the **same** file. Git adapts by **collapsing** case when that applies—less noise locally, but **ambiguity** about the canonical spelling.
