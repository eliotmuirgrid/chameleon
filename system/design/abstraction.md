...
Abstractions **trade detail for clarity**. That trade is good when the hidden layer **behaves** the way the simple surface **promises**. When it does not, you get a [leaky abstraction](leaky/abstraction.md): hidden details break through and you must care about them anyway. [Separation of concerns](separation_of_concerns/concept.md) helps keep each layer nameable; [small packets](small/packet.md) of work or data are often what one abstraction wraps into a single named move.
